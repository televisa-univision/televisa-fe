...
import favicon from '../../assets/lasestrellas/favicon.ico';
 ...
 * @returns {JSX}
 */
export function LasEstrellasPage({ store, config }) {
  return (
    <Provider store={store}>
      <ErrorBoundary>
        ...
        <ContentTypeWrapper site={LAS_ESTRELLAS_SITE} />
...
LasEstrellasPage.getInitialProps = async (context) => {
  return setupPage(context, LAS_ESTRELLAS);
};

LasEstrellasPage.propTypes = {
...
const LasEstrellasPageWithRouter = withRouter(LasEstrellasPage);

export default withRedux(configureStore)(LasEstrellasPageWithRouter);
