...
describe('Article Infinity Scroll', () => {
  let settings;
  const duration = 500;

  before(() => {
    cy.fixture('article').then((fixture) => {
      settings = fixture;
      cy.server();
  ...
  describe('User Interface', () => {
    describe('Enhancement Layout', () => {
      it('Has a main article', () => {
        cy.window().then(win => win.scrollTo(0, 0));
        cy.get(settings.selectors.mainArticle).should('have.length.of.at.least', 1);
      });
    });
    ...
    describe('Infinity Scroll', () => {
      ...
      it.skip('should change the url when the scroll goes to the second article', () => {
        cy.scrollTo(0, 0);
        cy.url().then((url) => {
          cy.get(settings.selectors.main)
            .eq(1)
