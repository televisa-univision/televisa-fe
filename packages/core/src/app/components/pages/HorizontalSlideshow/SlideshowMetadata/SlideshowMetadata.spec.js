...
describe('SlideshowMetadata Spec', () => {
  ...
  it('should render all the metadata with one slide', () => {
    const props = { page: { slides: [{ content: { renditions: { original: {} } } }] } };
    const wrapper = shallow(<SlideshowMetadata {...props} />);
    expect(wrapper.find('meta')).toHaveLength(7);
  });
  it('should render all the metadata with one slide when video', () => {
    const props = {
      page: {
        slides: [
