...
describe('DarkCover', () => {
  it('should render without crashing', () => {
    ...
    ReactDOM.render(<DarkCover />, div);
  });
  it('renders using image object', () => {
    const wrapper = shallow(<DarkCover image={image} />);
    expect(wrapper.find('BackgroundImage').prop('image')).toEqual(image);
  });
  it('renders without image object', () => {
    const imageUrl = 'http://test.com/test.jpg';
    const wrapper = shallow(<DarkCover overrideImageUrl={imageUrl} />);
    expect(wrapper.find('BackgroundImage').prop('overrideImageUrl')).toEqual(imageUrl);
  });
});
