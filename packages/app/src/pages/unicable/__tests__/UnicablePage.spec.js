...
import { UnicablePage } from '../index.page';
...
describe('Canal5Page test', () => {
  ...
  it('should render correctly', () => {
    const wrapper = shallow(<UnicablePage {...initialProps} />);
    expect(wrapper).toHaveLength(1);
  });

  it('should set page data from getInitialProps', async () => {
    const props = await UnicablePage.getInitialProps(initialProps);
    let wrapper;

    await act(async () => {
      wrapper = mount(<UnicablePage store={initialProps.store} />);
    });
  ...
  it('should render error page when there is not pageData', async () => {
    ...
    const wrapper = mount(<UnicablePage store={initialProps.store} />);
