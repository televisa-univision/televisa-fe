...
const toPosix = require("serverless-nextjs-plugin/utils/pathToPosix");
const PluginBuildDir = require("serverless-nextjs-plugin/classes/PluginBuildDir");

class NextPage {
  constructor(pagePath, { customHandler, serverlessFunctionOverrides, routes } = {}) {
  ...
  get serverlessFunction() {
    ...
    configuration.package = this.getPackage();
  ...
  getPackage() {
    ...
    packageInclude.push(
      this.pagePath,
      this.pagePath.replace('.js', '.original.js'),
      path.posix.join(this.customHandler, "**", "*.js"),
      `!${path.posix.join(this.customHandler, "**", "*.test.js")}`
    );
