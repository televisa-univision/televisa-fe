...
/** @test {ReleatedContent} */
describe('RelatedContent Spec', () => {
  it('should renders without crashing', () => {
    const div = document.createElement('div');
    ReactDOM.render(<RelatedContent content={contentItems} />, div);
  });
  it('should render a MostViewedList', () => {
    const wrapper = shallow(<RelatedContent content={contentItems} />);
    expect(wrapper.find('NumericList').length).toBe(1);
  });
  it('should render empty div if content is empty', () => {
    const obj = {};
    const wrapper = shallow(<RelatedContent content={obj} />);
