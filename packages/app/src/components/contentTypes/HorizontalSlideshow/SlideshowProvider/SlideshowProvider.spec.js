...
import { SlideshowProvider, mapDispatchToProps } from '.';
...
describe('SlideshowProvider', () => {
  ...
  it('should not go to next slide if last slide', () => {
  ...
  it('should not go to previous slide if at the beginning and is not first slideshow', () => {
  ...
  it('should not go to next slide if at the end of an inline slideshow', () => {
    const wrapper = mount(<SlideshowProvider {...props} endSlideIndex={0} type="inline" />);
    const instance = wrapper.instance();
    const setActiveSlideSpy = jest.spyOn(instance, 'setActiveSlide');
    instance.goToNextSlide();
    expect(setActiveSlideSpy).not.toHaveBeenCalled();
  ...
  it('should automatically go to next slideshow if autoplay is on', () => {
    const onGoToNextSlideshow = jest.fn();
    ...
    const wrapper = mount(
      <SlideshowProvider
        ...
        endSlideIndex={4}
        isFinalSlideshow={false}
        onGoToNextSlideshow={onGoToNextSlideshow}
        nextSlideshow={{ uri: 'test' }}
      />,
    );
    const instance = wrapper.instance();
    ...
    expect(onGoToNextSlideshow).toHaveBeenCalled();
    expect(restartTimerSpy).toHaveBeenCalled();
  });

  it('should go to previous slideshow if first slide', () => {
    const onGoToPreviousSlideshow = jest.fn();
    ...
    const wrapper = mount(
      <SlideshowProvider
        ...
        endSlideIndex={4}
        isFinalSlideshow={false}
        onGoToPreviousSlideshow={onGoToPreviousSlideshow}
        previousSlideshow={{ uri: 'test' }}
    ...
    instance.triggerPreviousSlideshow();

    expect(onGoToPreviousSlideshow).toHaveBeenCalled();
    expect(restartTimerSpy).toHaveBeenCalled();
  });

  it('should not go to previous slideshow if stiching is disabled', () => {
    ...
    expect(onGoToPreviousSlideshow).not.toHaveBeenCalled();
...
describe('mapDispatchToProps', () => {
  it('should dispatch an action when onGoToNextSlideshow is called', () => {
    const onGoToNextorPreviousSlideshow = jest.spyOn(horizontalSlideshowActions, 'goToNextorPreviousSlideshow');
    const spyDispatch = jest.spyOn(store, 'dispatch');
    const receivedActions = mapDispatchToProps(spyDispatch);

    act(() => {
      receivedActions.onGoToNextSlideshow();
    });
    expect(spyDispatch).toBeCalled();
    expect(onGoToNextorPreviousSlideshow).toHaveBeenCalledWith();
  });

  it('should dispatch an action when onGoToPreviousSlideshow is called', () => {
    const onGoToNextorPreviousSlideshow = jest.spyOn(horizontalSlideshowActions, 'goToNextorPreviousSlideshow');
    const spyDispatch = jest.spyOn(store, 'dispatch');
    const options = { isGoingForward: false };
    jest.spyOn(store, 'getState').mockReturnValue({ horizontalSlideshow: { currentSlideshowIndex: 1 } });
    const receivedActions = mapDispatchToProps(spyDispatch);
    act(() => {
      receivedActions.onGoToPreviousSlideshow(options);
    });
    expect(spyDispatch).toBeCalled();
    expect(onGoToNextorPreviousSlideshow).toHaveBeenCalledWith(options);
  });
});
