...
import ExpertPost from '../ExpertPost/ExpertPost';
...
class Feed extends Component {
  ...
  constructor(props) {
    ...
    this.pageData = getKey(getPageData(Store), 'data', {});
  }
    ...
    if (isAfterPinned || isFirstPost) {
      return (
        <Fragment key={`first-post-${post.uid}`}>
          <div
            className={classnames(Styles.inProgress, 'uvs-font-a-bold')}
          >
            <Icon name="liveblogComment" size="xsmall" />{' '}
