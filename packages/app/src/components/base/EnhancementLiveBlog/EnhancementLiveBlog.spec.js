...
import EnhancementLiveBlog from '.';
...
describe('getEnhancement tests', () => {
  it('returns null if no type', () => {
    const wrapper = shallow(<EnhancementLiveBlog />);
  ...
  it('returns null for unknown types', () => {
    ...
    const wrapper = shallow(<EnhancementLiveBlog data={data} />);
    expect(wrapper.isEmptyRender()).toBe(true);
  });
  ...
  it('does not use Loadable for non-lazy-loaded externalcontent', () => {
    ...
    const wrapper = shallow(<div><EnhancementLiveBlog data={data} /></div>);
