...
describe('EndCard', () => {
  ...
  it('should track a click to related gallery - with event', () => {
    ...
    const node = Features.slideshows.horizontal.stitchingEnabled()
      // ContentCard gets wrapped with "WithRadioStation"
      ? 'WithRadioStation'
      : ContentCardGrid;
  ...
  it('should not track a click to related gallery - without event', () => {
    const node = Features.slideshows.horizontal.stitchingEnabled()
      ? 'WithRadioStation'
      : ContentCardGrid;
    spyOn(SlideshowTracker, 'track');
    ...
    jest.runAllTimers();
    expect(SlideshowTracker.track).toHaveBeenCalledTimes(0);
  });
  ...
  it('should render the correct horizontal slideshow desktop end card', () => {
    ...
    const wrapper = mount(<Provider store={Store}><EndCard {...props} slideshowType="horizontal" /></Provider>);
