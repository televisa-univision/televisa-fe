...
import { LasEstrellasPage } from '../index.page';
...
/**
 * @test {LasEstrellasPage}
 */
describe('LasEstrellasPage test', () => {
  beforeAll(async () => {
  ...
  it('should render correctly', () => {
    const wrapper = shallow(<LasEstrellasPage {...initialProps} />);
    expect(wrapper).toHaveLength(1);
  });

  it('should set page data from getInitialProps', async () => {
    const props = await LasEstrellasPage.getInitialProps(initialProps);
    let wrapper;

    await act(async () => {
      wrapper = mount(<LasEstrellasPage store={initialProps.store} />);
    });

    const LasEstrellasLayout = wrapper.find('LasEstrellasLayout');
    expect(LasEstrellasLayout.exists()).toBe(true);
  ...
  it('should render error page when there is not pageData', async () => {
    ...
    const wrapper = mount(<LasEstrellasPage store={initialProps.store} />);
