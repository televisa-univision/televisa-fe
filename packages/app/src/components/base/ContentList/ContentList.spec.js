...
describe('ContentList', () => {
  ...
  it('should send taboola notification', () => {
  ...
  describe('setItemRef', () => {
    it('should call setItemRef for the first item', () => {
      const setItemRefSpy = jest.spyOn(ContentList.prototype, 'setItemRef');
      const contents = [{}, {}, {}, {}];
      mount(
      ...
      expect(setItemRefSpy).toBeCalled();
    });

    it('should not add an item to the ref list if not dom element is passed', () => {
      const contents = [{}, {}, {}, {}];
      const wrapper = shallow(
        <ContentList contents={contents} limit={2} itemComponent={TestItem} />,
      ).instance();
      ...
      expect(wrapper.items).toHaveLength(0);
    });
