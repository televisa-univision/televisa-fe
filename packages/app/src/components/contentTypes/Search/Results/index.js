...
export const shouldRenderAd = (device, idx) => {
  ...
  return !!(
    ...
    && (idx === settings.firstAdPosition - 1
      || (idx && idx % (settings.adInterval + (settings.firstAdPosition - 1)) === 0))
  );
};
 ...
 */
const Results = (props) => {
  const { results, loading, device } = props;
...
/**
 * propTypes
 * @property {string} device - current device from the user client
 * @property {array} results - the results from content api
 * @property {bool} loading - is loading the content
 */
Results.propTypes = {
