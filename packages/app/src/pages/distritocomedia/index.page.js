...
import { DISTRITOCOMEDIA_SITE } from '@univision/fe-commons/dist/constants/sites';
...
import favicon from '../../assets/distritocomedia/favicon.ico';
import Scripts from '../../components/base/Scripts';

/**
 * Distrito Comedia Page
 ...
 * @returns {JSX}
 */
export function DistritoComediaPage({ store, config }) {
  return (
    <Provider store={store}>
      <ErrorBoundary>
        ...
        <ContentTypeWrapper site={DISTRITOCOMEDIA_SITE} />
...
DistritoComediaPage.getInitialProps = async (context) => {
  return setupPage(context, DISTRITO_COMEDIA);
};

DistritoComediaPage.propTypes = {
...
const DistritoComediaPageWithRouter = withRouter(DistritoComediaPage);

export default withRedux(configureStore)(DistritoComediaPageWithRouter);
