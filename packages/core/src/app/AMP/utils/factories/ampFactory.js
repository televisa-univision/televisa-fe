...
const ampFactory = {
  ...
  embedToAmp(objectData) {
    ...
    const instagramPostRegexp = /"https:\/\/www.instagram.com\/p\/([\s\S]*?)\//i;
    ...
    if (intagramEmbedRegexp.test(objectData.html)) {
      const match = instagramPostRegexp.exec(objectData.html);
      if (match) {
        ampHtml = (
          <amp-instagram
  ...
  getAmpHtmlLink(initialState) {
    const supportedTypes = ['article'];
    if (
      hasKey(initialState, 'data.type')
      && supportedTypes.includes(initialState.data.type)
      && initialState.data.enableForGoogleAmp
    ) {
      const cleanUri = toRelativeUrl(initialState.data.seo?.canonicalUrl || initialState.data.uri);
      return (
        <link
          rel="amphtml"
          href={toAbsoluteUrl(`/amp${cleanUri}`, initialState.domain)}
        />
      );
    }
    return null;
