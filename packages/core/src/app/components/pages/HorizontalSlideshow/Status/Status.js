...
class Status extends Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    const { activeSlideNumber } = nextProps;
    const { activeSlideNumber: activeSlideNumberProps } = this.props;

    if (!exists(activeSlideNumber) && activeSlideNumber !== 0 && exists(activeSlideNumberProps)) {
      this.prevSlideNumber = activeSlideNumberProps;
    }
  }
  ...
  render() {
    ...
    const classNames = classnames(className, {
      [Styles.isEndCard]: isEndOfSlideshow && exists(slides),
    });
